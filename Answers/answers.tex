...
Since the main body of the algorithm still uses the unmodified Viterbi algorithm, the complexity of that section is $O(k^2 \times n)$ as standard. The pre-computing of possible silent state sequences is the main difference from the normal procedure, so this is where a difference will be made.

Technically, the modified Viterbi being run for each state transition could run forever in a model where emission and transition probabilities of 1 are present. However, it is highly unlikely that real-world HMMs will incorporate probabilities of 1, so I believe this is not a problem. In a more realistic scenario, with a reasonable number of states with reasonable transition and emission probabilities, it is unlikely that it will reach sequences with greater lengths than one or two orders of magnitude.

The modified Viterbi algorithms will need to iterate over [all states, for each state, for each sequence length], and is being run for [all end states, and for each start state]. Giving the sequence length before (all probabilities $\leq$ transition probability) for any start/end pair the name $s$, its time complexity is a function of:

$$k^4 \times \text{Average}(s)$$

The value of this final item is difficult to compute, as it relies on nuances relating to the distributions of transition and emission probabilities, and is really an average over all combinations of start and end states. However, as mentioned above, I believe that it will not reach unreasonable magnitude for a real-world HMM, so it is not a huge problem. Also, if there is a model with any excessively large number of iterations needed, that model would trivially have an extremely uneven distribution of probabilities. Therefore, some state pairs would need zero (or otherwise a very small number of) iterations to evaluate, which goes some way to offset this individual complexity.

To summarise, both $k$ and the average sequence length required to evaluate the silent state sequences will be relatively small for most reasonable cases. It is worth noting that the large variable, the overall sequence length $n$, has no bearing on this process - meaning this preprocessing section will take the same time even for extreme sequence lengths.

\hfill

The space complexity of the first section is also fairly reasonable. The matrix that stores the most likely silent state sequences for each transition will be of size $k^2 \times \text{(Average OR Maximum)}(s)$, depending on whether a linked list or fixed array is used for the state sequences. The trellis of silent state probabilities will never exceed a factor of $k \times \text{Maximum}(s)$.

For the main Viterbi section, we once again have the usual space complexity of $k \times n$ for the main trellis, the emission matrix at $k \times \text{Number of symbols}$, and the transition matrix at $k^2$. However, we also need to store the silent state sequences for each transition for backtracking purposes, at the above size of $k^2 \times \text{(Average OR Maximum)}(s)$.
...
It is worth noting that $n$, for a typical HMM, will be far larger than $k$ or $s$, so the apparent complexity of the precomputation is less extreme than it may seem at a glance.
